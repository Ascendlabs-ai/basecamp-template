...
`0003_seed_categories.sql` has **no upstream counterpart**: it was written in
this repo, it contains rows and no schema, and upstream's baseline generator
neither produces nor knows about it. A three-way merge has nothing to say about
it, so the entry above exists to stop a future maintainer concluding from its
absence upstream that it was deleted there and should be dropped here. It is a
provisioning step — `supabase/README.md` step 1 lists it — and removing it
returns a freshly stamped app to an empty category list.

It is also the file to add rows to if starter data ever grows. It must never be
folded into `0001_baseline.sql`: that file is regenerated by `pg_dump
--schema-only`, its header promises no rows, and a hand-added INSERT there
disappears at the next regeneration with nothing to mark its passing.

The six harness changes to preserve in `boundary_mutations.sh`, none of which
exists upstream:
...
6. **PART 12, the Editor-path arm**, with `EXPECTED_EDITOR_CASES`, the `eran`
   ...
   easiest of the six to lose, because the psql arm keeps printing 96/96 without
   it and the total still looks like a plausible number.

Taking upstream's copy wholesale reverts all six. The worst is (3): upstream
...
It checks identity leaks, and it checks three things about the suite's shape:
`EXPECTED_CASES` against the `run_case` count — which predates the Editor-path
arm — plus, added with that arm, `EXPECTED_EDITOR_CASES` against the
`run_editor_case` count and the fact that PART 12 still builds CRLF fixtures for
both files and applies 0001 as one whole-file `-c`. Those three are designed
guards and will survive an upstream reword. Nothing in this repository asserts
anything about `setup ()`, the preflight or the whitelist split beyond those
three shape checks.
